...
func Find(w http.ResponseWriter, r *http.Request, p locationMaterialTypes.T_params, q *pgstore.Queries) {
	...
	location_material, err := q.FindLocationMaterialById(r.Context(), p.ID)

	if err != nil {
		helper.HandleErrorMessage(w, err, "Location material")
		return
	}
